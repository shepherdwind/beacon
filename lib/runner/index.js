...
PhantomRunner.prototype = {
  ...
  run: function(url, callback){
    ...
    log.debug(url, 'receive task, uid: '+ uid);
   ...
   */
  status: function(){
    return this.process.map(function(runner){
      return {port: runner.port, status: runner.status};
    });
  },
  ...
  _process: function(process){
    ...
    if (task){
      ...
      process.status = 'busy';
      log.debug(task.url, 'start run task ' + task.uid + ' @' + port);
  ...
  _startProcess: function(){
    ...
    log.debug('server', 'start server on port: ' + port);

    runner.stdout.once('data', function(){
      _process.status = 'free';
      log.debug('server', 'server on port: ' + port + ' is on');
      self._run();
    });
